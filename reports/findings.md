...
基於三次測試（Run 1/2 自訂參數 + Run 3 官方參數）的綜合結論。所有模型為 4-bit MLX 量化版本。
...
| Profile | 模型 | 適用場景 | 關鍵參數 |
|---------|------|---------|---------|
| **fast** | 0.8B-4bit | 路由、分類、簡單問答、摘要、閱讀理解 | temp=0.7, pp=1.5 |
| **default** | 9B-4bit | 翻譯、程式碼、創意寫作、Tool Calling | temp=0.7, pp=1.5 |
| **reasoning** ⭐ | 9B-4bit | 邏輯推理、數學推理 | temp=1.0, top_k=40, pp=2.0 |

### 不推薦常規使用的 Profile

| Profile | 原因 | 替代方案 |
|---------|------|---------|
| **thinking** | `<think>` 標籤在 4-bit 量化下生成率 ~50%，失敗時 thinking loop | 用 **reasoning** |
| **thinking-code** | 同上，但低溫（temp=0.6）可能較穩定，待驗證 | 用 **default** |
